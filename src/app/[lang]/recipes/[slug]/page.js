...
function generateSEOContent(
    ...
    const localsIngredients = { "en": "ingredients of", "es": "ingredientes de", "it": "ingredienti del" };
    const localsCalories = { "en": "how many calories has", "es": "cuantas calorías tiene", "it": "quante calorie ha" };
...
export async function generateMetadata({ params, searchParams }) {
    ...
    const getCanonicalSlug = (name, language) => {
        return name?.normalize('NFD') // Normalize to decompose accented characters
        ...
        alternates: {
            ...
            languages: {
                ...Object.keys(locals).reduce((acc, language) => {
                    acc[language] = `${baseUrl}/${language}/recipes/${getCanonicalSlug(cocktailName, language)}?id=${cocktailId}`;
                    return acc;
                }, {}),
                'x-default': `${baseUrl}/en/recipes/${getCanonicalSlug(cocktailName, 'en')}?id=${cocktailId}`
            }
            ...
            <div>
                ...
                <div className="recipe-post-container">
                    <div className="recipe-title">
                        <h1>{t('recipe.headerRecipeH1')} {upperCocktail}</h1>
                    </div>
                                ...
                                <div className="side-image-container">
                                    {iba && (
                                        ...
                                        <div className="main-info-container">
                                            <div className="main-info-label">{t('recipe.recognition')}</div>
                                    ...
                                    {source && (
                                        <div className="main-info-container">
                                            <div className="main-info-label">{t('recipe.source')}</div>
                                            <div className="main-info-box">
                                                {source ? source : (origin ? origin : t('recipe.unknown'))}
                                            </div>
                                        </div>
                                    )}
                                    <RatingDisplay ratingAvg={ratingAvg} votes={ratingCount} message={t('blog.alert_rating_msm')} />
                                </div>
                                {/* QR CODE GENERATION */}
                                <div className="side-image-container">
                    ...
                    {postRecord && postRecord[lang]?.slug && (
                        <div className="blog-banner-container">
                            <div className="btnBanner">
                                <Link href={`/${lang}/blog/${postRecord[lang].slug}`} className="cta-button-red">
                                    {t('recipe.blog-link')}
                                </Link>
                            </div>
                        </div>
